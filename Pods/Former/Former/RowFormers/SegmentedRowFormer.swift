...
: BaseRowFormer<T>, Formable where T: SegmentedFormableRow {
    ...
    open override func cellInitialized(_ cell: T) {
        ...
        cell.formSegmented().addTarget(self, action: #selector(SegmentedRowFormer.valueChanged(_:)), for: .valueChanged)
    }
    ...
    fileprivate final var onSegmentSelected: ((Int, String) -> Void)?
    fileprivate final var titleColor: UIColor?
    
    fileprivate dynamic func valueChanged(_ segment: UISegmentedControl) {
        if enabled {
