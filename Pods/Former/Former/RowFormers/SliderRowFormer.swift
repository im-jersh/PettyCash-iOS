...
: BaseRowFormer<T>, Formable where T: SliderFormableRow {
    ...
    open override func cellInitialized(_ cell: T) {
        ...
        cell.formSlider().addTarget(self, action: #selector(SliderRowFormer.valueChanged(_:)), for: .valueChanged)
    }
    ...
    fileprivate final var onValueChanged: ((Float) -> Void)?
    fileprivate final var displayTextFromValue: ((Float) -> String)?
    fileprivate final var adjustedValueFromValue: ((Float) -> Float)?
    fileprivate final var titleColor: UIColor?
    fileprivate final var displayColor: UIColor?
    
    fileprivate dynamic func valueChanged(_ slider: UISlider) {
        let displayLabel = cell.formDisplayLabel()
